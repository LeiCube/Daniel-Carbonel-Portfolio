...
const ICON = {
  ...
  chevronDown: '<svg class="icon photo-shoot-chevron" viewBox="0 0 24 24"><path d="m6 9 6 6 6-6"/></svg>',
};
...
function renderGallery() {
  ...
  if (slug === "reels") {
    ...
    photoShoots.forEach((shoot, i) => {
      const contentId = `shoot-content-${i}`;
      ...
      header.type = "button";
      header.className = "photo-shoot-header";
      header.setAttribute("aria-expanded", "true");
      header.setAttribute("aria-controls", contentId);
      header.innerHTML = `
        <h2 class="photo-shoot-name">${shoot.name}</h2>
        <span class="photo-shoot-header-right">
      ...
      content.className = "photo-shoot-content";
      content.id = contentId;
      section.appendChild(content);

      body.appendChild(section);
      renderMasonryGallery(content, shoot.items, { aspectW: 3, aspectH: 2 });
    });
  }
}
